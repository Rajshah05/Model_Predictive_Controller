...
try:
    ...
    # -----------X-ref--------------------
    # cvel = norm_2([vehicle.get_velocity().x, vehicle.get_velocity().y])
    # ttnwp = 2/cvel
    ...
    # -----------------curvature----------------
    # endpoint = world.get_map().get_waypoint(carla.Location(x=-65.7, y=188.7, z=0.0), project_to_road=True, lane_type=carla.LaneType.Driving).transform
    # print(endpoint)
    ...
    # ------------------straight-------------------
    # endpoint = world.get_map().get_waypoint(carla.Location(x=-53.375589, y=197.064423, z=1.843102), project_to_road=True, lane_type=carla.LaneType.Driving).transform
    # print(endpoint)
    # world.debug.draw_string(endpoint.location, 'O', draw_shadow=False,
    #                                     color=carla.Color(r=255, g=0, b=0), life_time=50,
    #                                     persistent_lines=True)
    ...
    rhs = vertcat((v*cos(theta+(atan((l_r*tan(strang*1.22))/(l_f + l_r))))), v*sin(theta+(atan((l_r*tan(strang*1.22))/(l_f + l_r)))), ((v/l_r)*(sin(atan((l_r*tan(strang*1.22))/(l_f + l_r))))), thr*16)
    ...
    # Q = diag(SX([99999,99999,1000,2])) #128  [4000,4000,1000,2]
    # R = diag(SX([0,0])) #128
    ...
    for i in range(0,4*(N+1),4):
        ...
        ubgv.append(405)
        ubgv.append(15)
    # args = {'lbg':-99999, 'ubg':99999, 'lbx':lbxv, 'ubx':ubxv}
    u0 = (DM.zeros(2*N,1))
    # sim_tim = 20
    u_cl = []
    ...
    # r_theta = startpoint.rotation.yaw
    # if r_theta < 0:
    #     r_theta = 360 - abs(r_theta)

    # if endpoint.rotation.yaw < 0:
    #     endpoint.rotation.yaw = 360 - abs(endpoint.rotation.yaw)

    # x0 = np.transpose([startpoint.location.x, startpoint.location.y, r_theta, 0])
    # xs = np.transpose([endpoint.location.x, endpoint.location.y, r_theta, 0]) #-90.156235*pi/180
    # p =  np.transpose([startpoint.location.x, startpoint.location.y, r_theta, 0, endpoint.location.x, endpoint.location.y, r_theta, 0])
    ...
    c=0
    p =  np.transpose([startpoint.location.x, startpoint.location.y, contheta(startpoint.rotation.yaw), 0,endpoint.location.x, endpoint.location.y, contheta(startpoint.rotation.yaw), 3])
    mpciter = 0
    
    # MPC LOOP
    # while (norm_2(x0[0:2]-xs[0:2]))>0.5:
    while c < len(wps):
        # r_theta = startpoint.rotation.yaw
        # if r_theta < 0:
        #     r_theta = 360 - abs(r_theta)
        ...
        #     print(c)
        #     print(len(wps))
        #     endpoint = wps[c+1].transform
        #     c +=1
        # print((norm_2(x0[0:2]-p[4:6])))
        # print(c)
        ...
        for k in range(N):
            ...
            world.debug.draw_string(carla.Location(x=float(ff_value[0,k]), y=float(ff_value[1,k]),z=0.0), 'O', draw_shadow=False,
                                        ...
                                        persistent_lines=True)
        # xx1[:,0:2,mpciter] = ff_value.T
        u_cl.append(u[0,:])
        # print("throttle",float(u[0,0]),"strang", float(u[0,1]))
        # print(u[0,0], u[0,1])
        ...
        x0 = np.transpose([vehicle.get_transform().location.x, vehicle.get_transform().location.y, contheta(u_theta), norm_2([vehicle.get_velocity().x, vehicle.get_velocity().y])])
